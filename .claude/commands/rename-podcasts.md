...
- すでに `#XX_タイトル` 形式（リネーム済み）のファイル名がついているものはスキップする
...
### 4. リネーム実行
リネーム形式: `#XX_メインタイトル ― サブタイトル.m4a`（番号とタイトルの間はアンダースコア）

- episode_prompts.md のフルタイトル（サブタイトル含む）を使用する
- ファイル名に使えない `/` は全角 `／` に変換する
- 例: `#01_2進数と小数変換 ― コンピュータの数の扱い方.m4a`
- 例: `#07_待ち行列理論 ― M／M／1モデルで行列を分析する.m4a`

リネーム前に全マッピング一覧を表示して、ユーザーの確認を取ってから実行すること。

### 5. 重複チェック
- 同じエピソード番号に対応するファイルが複数ないか確認する（リネーム済みファイルと新たにマッピングされたファイルが同じ番号を持つ場合など）
- 重複がある場合はファイル名・サイズを一覧表示してユーザーに報告する
- ユーザーの確認を取ってから不要な方を削除する

### 6. 検証
- `ls podcast/` でファイルが `#XX_` 形式で番号順にリストされることを確認
